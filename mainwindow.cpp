...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        auto res1 = [&]{return concurRace1->DoWork(&number, false, ui->sb_initNum->value());};
        auto res2 = [&]{return concurRace2->DoWork(&number, false, ui->sb_initNum->value());};
        QtConcurrent::run(res1).then(res2);
